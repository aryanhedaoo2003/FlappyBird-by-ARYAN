...
def welcomeScreen():
    ...
    message_y = int(screen_height*0.002)#0.14
    title_x = int((screen_width - game_images['message'].get_width())/500)
    title_y = int(screen_height*0.07)#0.05
    base_x = 1#5
    while True:
...
def getRandomPipe():
    pipeHeight = game_images['pipe'][0].get_height()    
    offset = screen_height/1.8 
    y2 = offset + random.randrange(0, int(screen_height - game_images['base'].get_height() - 1*offset))
...
if __name__ == "__main__":
    ...
    game_images['message'] = pygame.image.load('gallery/images/message.jpg').convert_alpha()
    game_images['base'] = pygame.image.load('gallery/images/base.jpeg').convert_alpha()
    game_images['pipe'] = (
